...
export default function StudentSchedule() {
  ...
  const stats = [
    ...
    {
      label: "Daily Avg.",
      value: Math.round(totalClasses / DAYS.length) || 0,
      icon: <Clock size={18} />,
      color: "#f59e0b",
      text: "text-warning",
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        <p className="page-subtitle">Your weekly timetable and class timings.</p>
      </div>
              ...
              >
                ...
                <div className="w-full h-1 rounded-full mt-1" style={{ background: `${color}30` }}>
                  <div
                    ...
                    style={{
                      width: `${totalClasses ? (dayClasses.length / Math.max(...DAYS.map(d => schedule.filter(s => s.day === d).length))) * 100 : 0}%`,
                      background: color,
                    }}
                  />
                </div>
              ...
              <div key={cls.id} className="card flex items-center gap-5 py-4">
                ...
                <div className="flex flex-1 md:gap-4 flex-col md:flex-row">
                  ...
                  <div className="flex gap-2 md:gap-0 md:flex-col items-center shrink-0 w-16">
                    <span className="text-sm font-semibold text-text">{cls.start_time}</span>
                    <span className="text-xs text-faint">to</span>
                    <span className="text-sm font-semibold text-text">{cls.end_time}</span>
                  </div>
