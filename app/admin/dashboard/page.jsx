...
export default function AdminDashboard({basePath = "/admin"}) {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      // second step — fetch results using the exam name from first step
  ...
  // Class-wise student count for bar chart
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {statCards.map((s, i) => (
          <div key={i} className={`stat-card ${s.bg} border ${s.border} no-underline block`}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {[
            ...
            <Link
              key={i}
              href={a.href}
              className="flex sm:items-center gap-3 p-4 rounded-lg border flex-col sm:flex-row border-border bg-surface hover:bg-bg hover:ring hover:ring-surface-2 no-underline transition-all duration-200"
            >
              <div
                className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                style={{ background: `${a.color}18`, color: a.color, boxShadow: `0 0 1px 1px ${a.color}40` }}
              >
                {a.icon}
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            {notices.map(n => (
              <div key={n.id} className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg bg-bg border border-border">
