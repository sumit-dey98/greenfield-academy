# Greenfield Academy — Database Restore Instructions

## Prerequisites
- Access to a fresh Supabase project
- The three SQL files: `tables.sql`, `constraints.sql`, `policies.sql`
- Access to the Supabase SQL editor (Project → SQL Editor)
...
- The superadmin Auth user **must** be created via the dashboard — it cannot be done in SQL
- If any step fails, check the error message — most common issue is a missing or mismatched UUID in Step 3
